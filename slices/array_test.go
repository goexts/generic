...
func runIndexAnyTests(t *testing.T, f func(s []rune, chars string) int, funcName string, testCases []BinOpTest) {
	for _, test := range testCases {
		a := []rune(test.a)
		actual := f(a, test.b)
		if actual != test.i {
...
// 		if got := Index([]byte(tt.in), []byte(string([]rune{tt.rune}))); got != tt.want {
// 			t.Errorf("Index(%q, %d) = %v; want %v", tt.in, tt.rune, got, tt.want)
// 		}
// 	}
//
// 	haystack := []rune("test世界")
...
func bmIndexRuneASCII(index func([]rune, rune) int) func(b *testing.B, n int) {
	return func(b *testing.B, n int) {
		buf := bmbuf[0:n]
		buf[n-1] = 'x'
		for i := 0; i < b.N; i++ {
...
func bmIndexRune(index func([]rune, rune) int) func(b *testing.B, n int) {
	return func(b *testing.B, n int) {
		buf := bmbuf[0:n]
		utf8.EncodeRune([]byte(string(buf[n-3:])), '世')
		for i := 0; i < b.N; i++ {
			j := index([]rune(string(buf)), '世')
			if j != n-3 {
				b.Fatal("bad index", j)
			}
		}
...
func TestFieldsFunc(t *testing.T) {
	for _, tt := range fieldstests {
		a := FieldsFunc([]rune(tt.s), IsSpace[rune])
		result := sliceOfString(a)
		if !eq(result, tt.a) {
	...
	for _, tt := range fieldsFuncTests {
		...
		if string(b) != tt.s {
			t.Errorf("slice changed to %s; want %s", string(x), tt.s)
		}
		if len(tt.a) > 0 {
			if want := tt.a[len(tt.a)-1] + "z"; string(x) != want {
				t.Errorf("last appended result was %s; want %s", string(x), want)
...
var upperTests = []StringTest{
	{"", []rune("")},
	{"ONLYUPPER", []rune("ONLYUPPER")},
	{"abc", []rune("ABC")},
	{"AbC123", []rune("ABC123")},
	{"azAZ09_", []rune("AZAZ09_")},
	{"longStrinGwitHmixofsmaLLandcAps", []rune("LONGSTRINGWITHMIXOFSMALLANDCAPS")},
	{"long\u0250string\u0250with\u0250nonascii\u2C6Fchars", []rune("LONG\u2C6FSTRING\u2C6FWITH\u2C6FNONASCII\u2C6FCHARS")},
	{"\u0250\u0250\u0250\u0250\u0250", []rune("\u2C6F\u2C6F\u2C6F\u2C6F\u2C6F")}, // grows one byte per char
	{"a\u0080\U0010FFFF", []rune("A\u0080\U0010FFFF")},                           // test utf8.RuneSelf and utf8.MaxRune
}
...
var lowerTests = []StringTest{
	...
	{"longStrinGwitHmixofsmaLLandcAps", []rune("longstringwithmixofsmallandcaps")},
	{"LONG\u2C6FSTRING\u2C6FWITH\u2C6FNONASCII\u2C6FCHARS", []rune("long\u0250string\u0250with\u0250nonascii\u0250chars")},
	{"\u2C6D\u2C6D\u2C6D\u2C6D\u2C6D", []rune("\u0251\u0251\u0251\u0251\u0251")}, // shrinks one byte per char
	{"A\u0080\U0010FFFF", []rune("a\u0080\U0010FFFF")},                           // test utf8.RuneSelf and utf8.MaxRune
}
...
// Execute f on each test case.  funcName should be the name of f; it's used
// in failure reports.
func runStringTests(t *testing.T, f func([]rune) []rune, funcName string, testCases []StringTest) {
...
// 	m = Map(minRune, []rune(tenRunes(unicode.MaxRune)))
// 	expect = a
// 	if string(m) != expect {
...
//
// 	// 4. Rot13^2
// 	m = Map(rot13, Map(rot13, []rune("a to zed")))
// 	expect = "a to zed"
// 	if string(m) != expect {
// 		t.Errorf("rot13: expected %q got %q", expect, m)
// 	}
...
// 		return -1
// 	}
// 	m = Map(dropNotLatin, []rune("Hello, 세계"))
// 	expect = "Hello"
// 	if string(m) != expect {
...
func TestToValidUTF8(t *testing.T) {
	for _, tc := range toValidUTF8Tests {
		got := ToValidUTF8Rune([]rune(tc.in), []rune(tc.repl))
		if !Equal(got, []rune(tc.out)) {
			t.Errorf("ToValidUTF8(%q, %q) = %q; want %q", tc.in, tc.repl, got, tc.out)
		}
	}
}

func TestTrimSpace(t *testing.T) { runStringTests(t, TrimSpaceRune, "TrimSpaceRune", trimSpaceTests) }
...
func TestTrim(t *testing.T) {
	toFn := func(name string) (func([]rune, []rune) []rune, func([]rune, []rune) []rune) {
		switch name {
		case "Trim":
			return Trim[[]rune], nil
		case "TrimLeft":
			return TrimLeft[[]rune], nil
		case "TrimRight":
			return TrimRight[[]rune], nil
		case "TrimPrefix":
			return nil, TrimPrefix[[]rune]
		case "TrimSuffix":
			return nil, TrimSuffix[[]rune]
		default:
	...
	for _, tc := range trimTests {
		...
		var actual string
		if f != nil {
			actual = string(f([]rune(tc.in), []rune(tc.arg)))
		} else {
...
func TestTrimFunc(t *testing.T) {
	for _, tc := range trimFuncTests {
		trimmers := []struct {
			...
			{"TrimFunc", TrimFunc[[]rune], tc.trimOut},
			{"TrimLeftFunc", TrimLeftFunc[[]rune], tc.leftOut},
			{"TrimRightFunc", TrimRightFunc[[]rune], tc.rightOut},
		}
...
// 		if s := string(Title([]rune(tt.in))); s != tt.out {
...
func TestToTitle(t *testing.T) {
	for _, tt := range ToTitleTests {
		if s := string(ToTitleRune([]rune(tt.in))); s != tt.out {
...
// 	{[]rune("abc"), "xyz", false},
// 	{[]rune("abc"), "xcz", true},
// 	{[]rune("a☺b☻c☹d"), "uvw☻xyz", true},
// 	{[]rune("aRegExp*"), ".(|)*+?^$[]", true},
// 	{[]rune(dots + dots + dots), " ", false},
// }
//
...
func BenchmarkToValidUTF8(b *testing.B) {
	tests := []struct {
		...
		{"Valid", []rune("typical")},
		{"InvalidASCII", []rune("foo\xffbar")},
		{"InvalidNonASCII", []rune("日本語\xff日本語")},
	}
	replacement := []rune("\uFFFD")
	b.ResetTimer()
	for _, test := range tests {
		b.Run(test.name, func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				ToValidUTF8Rune(test.input, replacement)
			}
		})
	}
}
...
// 	x := Repeat([]rune{'#'}, 2048) // Never matches set
...
func BenchmarkTrimASCII(b *testing.B) {
	...
	for k := 1; k <= 4096; k <<= 4 {
		for j := 1; j <= 16; j <<= 1 {
			b.Run(fmt.Sprintf("%d:%d", k, j), func(b *testing.B) {
				x := Repeat([]rune(cs[:j]), k) // Always matches set
				for i := 0; i < b.N; i++ {
					Trim(x[:k], []rune(cs[:j]))
				}
			})
